...
X_train=pd.read_csv("data/embeddings/parametric_UMAP/parametric_UMAP_embeddings_10D_train.csv", header=None)
all_features=pd.read_csv("data/embeddings/parametric_UMAP/parametric_UMAP_embeddings_10D_all.csv", header=None)

#### train model for TSS effluent 
split_index =  len(X_train) 
y_train=TSSeffluent_labels[:split_index]

rf = RandomForestRegressor(n_estimators=500,  # Number of trees
                           max_depth=30,      # Maximum depth of the trees
                           random_state=42)  # Ensures reproducibility
rf.fit(X_train, y_train)

y_pred = rf.predict(all_features)
...
eff_TSS_hybridmodel=mech_model_output_reindex['.SST_1.X_Out']+average_error_preds
...
rf2 = RandomForestRegressor(n_estimators=500,  # Number of trees
                           max_depth=30,      # Maximum depth of the trees
                           random_state=42)  # Ensures reproducibility
rf2.fit(X_train, y_train2)

y_pred = rf2.predict(all_features)
