...
extrainput=pd.read_csv('data/settling_data/filtered_data/MLSSandRASrate_normalized.csv', index_col=0)
...
for folder_name in all_image_folders:
    ...
    for image_file in images_list_embeddings:
        try:
            ...
            aggregated_features=np.append(aggregated_features, extrainput['sensormlssweeklyaverage'].loc[folder_name].item())
...
rf_eff = RandomForestRegressor(n_estimators=15,
                               max_depth=5,
                               random_state=42,
...
plt.plot(eff_TSS_hybridmodel.iloc[train_range1], '.-', label='Model predictions (train)', color='orange')
plt.plot(eff_TSS_hybridmodel.iloc[train_range2], '.-', color='orange')
plt.plot(eff_TSS_hybridmodel.iloc[test_indices_folders], '.-', label='Model predictions (test)', color='red')

# Plot Std Dev Band for Train – Part 1
plt.fill_between(eff_TSS_hybridmodel.index[train_range1],
                 eff_TSS_hybridmodel_lower[train_range1],
                 eff_TSS_hybridmodel_upper[train_range1],
                 color='orange', alpha=0.2, zorder=1)

# Plot Std Dev Band for Train – Part 2
plt.fill_between(eff_TSS_hybridmodel.index[train_range2],
                 eff_TSS_hybridmodel_lower[train_range2],
                 eff_TSS_hybridmodel_upper[train_range2],
                 color='orange', alpha=0.2, zorder=1)

# Plot Std Dev Band for Test
plt.fill_between(eff_TSS_hybridmodel.index[test_indices_folders],
                 eff_TSS_hybridmodel_lower[test_indices_folders],
                 eff_TSS_hybridmodel_upper[test_indices_folders],
...
plt.plot(ras_TSS_hybridmodel.iloc[train_range1], '.-', label='Model predictions (train)', color='orange')
plt.plot(ras_TSS_hybridmodel.iloc[train_range2], '.-', color='orange')
plt.plot(ras_TSS_hybridmodel.iloc[test_indices_folders], '.-', label='Model predictions (test)', color='red')

# Plot Standard Deviation Band (Train) - use iloc
plt.fill_between(ras_TSS_hybridmodel.index[train_range1],
                 ras_TSS_hybridmodel_lower.iloc[train_range1],
                 ras_TSS_hybridmodel_upper.iloc[train_range1],
                 color='orange', alpha=0.2, zorder=1)
...
plt.fill_between(ras_TSS_hybridmodel.index[test_indices_folders],
                 ...
                 color='red', alpha=0.2, zorder=1)
# Plot Std Dev Band for Train – Part 2
plt.fill_between(ras_TSS_hybridmodel.index[train_range2],
                 ras_TSS_hybridmodel_lower[train_range2],
                 ras_TSS_hybridmodel_upper[train_range2],
                 color='orange', alpha=0.2, zorder=1)
