...
all_labels_eff_list = []
all_labels_ras_list = []
# all_labels_ras_actual_list = [] # If needed for direct prediction later
...
folder_idx_counter = 0 # Index corresponding to error arrays
for folder_name in all_image_folders:
    ...
    for image_file in images_list_embeddings:
        try:
            ...
            embedding = torch.load(img_path).cpu().numpy() # Shape (C, H, W) e.g. (8, 24, 32)

            # --- Feature Aggregation (as per user code) ---
            # mean/max/min per channel -> takes max over H and W dimensions
            ...
            # feature_folder_map.append(folder_idx_counter) # Store the original folder index for this feature
...
all_features_agg = np.array(all_features_list) # Shape (num_total_images, num_agg_features) e.g. (N, 8)
TSSeffluent_labels = np.array(all_labels_eff_list)
TSSras_labels = np.array(all_labels_ras_list)
feature_folder_map = np.array(feature_folder_map) # Index mapping features to PROCESSED folders
...
num_folders_processed = len(processed_folder_indices) # Number of folders we actually have data for

print(f"Total folders found: {num_folders_total}")
print(f"Folders successfully processed: {num_folders_processed}")
...
assert len(lab_measurements_reindex) == num_folders_processed # Check reindexed length

# --- Split Based on Time (Processed Folders) ---
# Use the number of *processed* folders for splitting
split_folder_index = round(SPLIT * num_folders_processed)
...
print(f"Splitting at processed folder index: {split_folder_index} (out of {num_folders_processed})")
...
############################################################
print("\n--- Training RF Model for TSS Effluent Error ---")

# Define the RandomForest model
rf_eff = RandomForestRegressor(n_estimators=100,
                               max_depth=10,
                               random_state=42,
                               n_jobs=-1) # Use all available CPU cores

# Train the model
print("Training Effluent RF model...")
rf_eff.fit(X_train, y_train_eff)

# --- Evaluate on Explicit Test Set ---
print("Evaluating Effluent RF model on Test Set:")
...
print("Predicting effluent error for all images...")
...
for folder_idx in range(num_folders_processed): # Iterate through processed folders
    ...
    if num_images_in_folder > 0:
        ...
        average_eff_error_preds.append(np.median(preds_for_folder)) # Median as per user code
        std_dev_eff.append(np.std(preds_for_folder))
        current_pred_index += num_images_in_folder
    else:
        # This case shouldn't happen if feature_folder_map is built correctly relative to num_folders_processed
        print(f"Warning: No effluent predictions found for processed folder index {folder_idx}. Appending NaN.")
...
plot_split_index = split_folder_index # Already calculated

print(f"Plotting TSS Effluent RF results using split index: {plot_split_index}")
...
########################################################
print("\n--- Training RF Model for TSS RAS Error ---")

# Define the second RandomForest model
rf_ras = RandomForestRegressor(n_estimators=500, # As per user's commented code
                               max_depth=50,     # As per user's commented code
                               random_state=42,
                               n_jobs=-1) # Use all available CPU cores

# Train the model (using the same X_train but different labels y_train_ras)
print("Training RAS RF model...")
rf_ras.fit(X_train, y_train_ras)

# --- Evaluate on Explicit Test Set ---
print("Evaluating RAS RF model on Test Set:")
...
# --- Predict on ALL Data for Hybrid Model ---
print("Predicting RAS error for all images...")
...
for folder_idx in range(num_folders_processed): # Iterate through processed folders
    ...
    if num_images_in_folder > 0:
        ...
        # Use mean for averaging as per user's original commented code for RAS
        average_ras_error_preds.append(np.mean(preds_for_folder))
...
# Use the same FOLDER-based split index for plotting consistency
# plot_split_index = split_folder_index # Already defined

print(f"Plotting TSS RAS RF results using split index: {plot_split_index}")
