...
X_train=pd.read_csv("data/embeddings/PCA/pca_embeddings_10D_train.csv", header=None)
all_features=pd.read_csv("data/embeddings/PCA/pca_embeddings_10D_all.csv", header=None)

#### train model for TSS effluent 
split_index =  len(X_train) 
y_train=TSSeffluent_labels[:split_index]

rf = RandomForestRegressor(n_estimators=200,  # Number of trees
...
rf2 = RandomForestRegressor(n_estimators=250,  # Number of trees
                           max_depth=15,      # Maximum depth of the trees
                           random_state=42)  # Ensures reproducibility
